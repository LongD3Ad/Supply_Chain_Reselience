...
# MODIFIED SIGNATURE: Added _gemini_model_instance
def get_gemini_recommendations(_data_summary, _context_text="", _gemini_model_instance=None):
    """Generates recommendations using a provided Gemini model instance.
       Underscore prefix tells st.cache_data to ignore this argument for hashing.
    """

    # NEW CHECK: Validate the passed instance
    if _gemini_model_instance is None:
        logging.error("get_gemini_recommendations called without a valid _gemini_model_instance.")
        # Return error structure matching Recommendation model
        return [{"priority": 0, "category": "Error", "text": "AI Model instance not provided to recommendation function."}]

    # REMOVED: The check for global GEMINI_ENABLED / gemini_model
    ...
    **Begin Analysis and Recommendations:**
    """
    try:
        logging.info("Sending recommendation request to Gemini API via passed instance.")
        # REMOVED: The check if global gemini_model is None

        # MODIFIED: Use the passed instance with the underscore
        response = _gemini_model_instance.generate_content(prompt)

        # (Keep your robust response handling logic)
        response_text = ""
        if response.parts: response_text = ''.join(part.text for part in response.parts)
             ...
             if block_reason: raise ValueError(f"Response blocked by API: {block_reason}")
             else: raise ValueError("Received unexpected or empty response from Gemini.")

        if not response_text:
             logging.warning("Gemini returned empty text for recommendations.")
             return [{"priority": 1, "category": "Info", "text": "AI analysis complete, but no specific recommendations generated."}]

        logging.info("Received recommendation response from Gemini. Parsing.")
        # Assume parse_recommendations is correct and returns list of dicts
        parsed_recs = parse_recommendations(response_text)

        if not parsed_recs:
             logging.warning("Failed to parse recommendations structure. Returning raw response.")
             return [{"priority": 0, "category": "Parsing Error", "text": "Could not parse recommendations. Raw AI response:\n" + response_text}]

        return parsed_recs

    except Exception as e:
        logging.error(f"Error calling Gemini API via instance or processing rec response: {e}", exc_info=True)
