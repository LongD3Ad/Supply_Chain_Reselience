...
DEFAULT_SIM_CONFIG = {
    ...
    "financial_defaults": {
        # Costs are per unit per week unless otherwise specified
        "holding_cost_unit_weekly": 0.50,       # Example: $0.50 per unit per week for inventory
        "stockout_cost_unit": 10.00,            # Example: $10.00 per unit of unmet demand (lost margin/penalty)
        "base_logistics_cost_unit": 2.00,       # Example: Base shipping cost $2.00 per unit
        "production_cost_unit": 5.00            # Example: Base production cost $5.00 per unit
        # Add others as needed, e.g., expedited shipping multiplier, fixed facility costs (more complex)
    }
}
