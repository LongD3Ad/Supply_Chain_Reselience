...
# (Copy the entire DEFAULT_SIM_CONFIG dictionary from your original script here)
DEFAULT_SIM_CONFIG = {
    "factories": [
        {
...
def load_simulation_config(config_path=None):
    ...
    # Use the CONFIG_PATH defined above if config_path arg is None
    effective_path = config_path if config_path else CONFIG_PATH
    # Attempt to make path absolute based on this file's location if it's relative
    if not os.path.isabs(effective_path):
         effective_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', effective_path)) # Go up one level from src/
    ...
    if effective_path and os.path.exists(effective_path):
        ...
        # Optionally add a warning if a specific path was provided but not found
        if config_path:
...
# This avoids calling load_simulation_config directly everywhere
@st.cache_data # Cache the loaded config itself
def get_config():
    return load_simulation_config() # Uses default path logic within load_simulation_config